...
function updateTimer() {
  ...
  const elapsed = effectiveNow - currentSession.adjustedStart - currentSession.pausedDuration;
...
function shiftStart(minutes) {
  ...
  currentSession.pausedDuration = Math.max(0, currentSession.pausedDuration);
  syncSlider();
  updateTimer();
}
...
function applySlider(value) {
  ...
  currentSession.adjustedStart = currentSession.originalStart + minutes * 60 * 1000;
  syncSlider();
  updateTimer();
}
...
startSlider.addEventListener('input', (event) => {
  ...
  const preview = new Date(currentSession.originalStart + minutes * 60 * 1000);
  sliderValue.textContent = `予定: ${formatTime(preview)}（差 ${formatDiff(minutes)}）`;
});
