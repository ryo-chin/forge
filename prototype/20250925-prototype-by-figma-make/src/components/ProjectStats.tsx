...
export function ProjectStats({ projects, taskHistory }: ProjectStatsProps) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    projects.forEach((project) => {
      const dayTasks = taskHistory.filter(
        (task) =>
          task.projectId === project.id && task.startTime.toDateString() === date.toDateString(),
      );
            ...
            <TableBody>
              {budgetVsActualData.map((day, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell
                    className={`text-right ${day.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {day.variance >= 0 ? '+' : ''}
                    {formatDuration(Math.abs(day.variance) * 60)}
                  </TableCell>
                  <TableCell
                    className={`text-right ${day.variancePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {day.variancePercent >= 0 ? '+' : ''}
                    {day.variancePercent.toFixed(1)}%
                  </TableCell>
            ...
            {projectStats.map((project) => {
              ...
              return (
                <div key={project.id} className="space-y-2">
                  ...
                  <Progress value={(project.totalDuration / maxTime) * 100} className="h-2" />
                  <div className="flex justify-between text-xs text-muted-foreground">
                    <span>Daily budget: {formatDuration(project.dailyBudget * 60)}</span>
                    <span>
                      Weekly: {formatDuration(project.recentDuration)} /{' '}
                      {formatDuration(weeklyBudget * 60)} ({budgetProgress.toFixed(0)}%)
                    </span>
                  </div>
                </div>
              );
