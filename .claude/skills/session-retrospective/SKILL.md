...
description: セッション終了時の振り返りを実施し、失敗をガードレール（司法）に変換する。AI駆動開発の生産性100xを実現するための改善サイクルを回す。使用タイミング: セッション終了時、「振り返り」「retrospective」と言われた時、実装完了後に改善点を洗い出したい時、同じ失敗を繰り返したくない時
---
